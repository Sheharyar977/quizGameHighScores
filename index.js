var timerKey = 'timer';
var sOriginalTime = 1 * 60 * 5; //4 minutes
...
function addScore() {
    ...
    for (let i = 0; i < hScores.length; i++) { 
        if (myHighScore.initials == hScores[i].initials) {
            let highScoresDiv = document.getElementById('high-scores');
            let Ctable = document.createElement('table')
            let tablerow1 = document.createElement('tr') ; 
            let tablerowrank = document.createElement('td');
            let tablerowscore = document.createElement('td');
            let tablerowinitals = document.createElement('td');
            tablerow1.appendChild( tablerowrank);
            tablerow1.appendChild(tablerowscore);
            tablerow1.appendChild(tablerowinitals); 
            tablerowrank.innerHTML = `Current rank: ${i+1}` ; 
            console.log(tablerowrank.innerHTML);
            tablerowscore.innerHTML = `Current Score: ${hScores[i].score}`; 
            tablerowinitals.innerHTML = `initials: ${hScores[i].initials}`;
            Ctable.appendChild(tablerow1); 
            highScoresDiv.appendChild(Ctable);
...
function showHighScores() {
    ...
    while(i<hScores.length ) {
        console.log("hScores.length", hScores[i].length)
        ...
        tablerow1.appendChild(tablerowscore);
        tablerow1.appendChild(tablerowinitals); 
        tablerowrank.innerHTML = i+1 ; 
        console.log(tablerowrank.innerHTML);
        tablerowscore.innerHTML = hScores[i].score; 
        tablerowinitals.innerHTML = hScores[i].initials;
        table.appendChild(tablerow1); 
        if (i>9) { 
            tablerow1.classList.add('hide'); 
